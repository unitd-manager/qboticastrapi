...
async function main() {
  ...
  try {
    for (const wpId of WP_PAGE_IDS) {
      ...
      const [strapiPages] = await conn.query('SELECT id FROM pages WHERE slug = ?', [wpSlug]);
      if (strapiPages.length === 0) {
        console.log(`  No Strapi page found for slug ${wpSlug}`);
        continue;
      }
      const strapiPageId = strapiPages[0].id;
      console.log(`  Found Strapi page ${strapiPageId} (slug ${wpSlug})`);
      
      const acf = await getPostMeta(conn, wpId);
      console.log(`  Got ${Object.keys(acf).length} ACF fields`);

      await conn.query('UPDATE pages SET acf = ? WHERE id = ?', [JSON.stringify(acf), strapiPageId]);
